...
namespace UniversalPay\Payment\Helper;

use UniversalPay\Payment\Helper\sdk\lib\Configurable;
use UniversalPay\Payment\Helper\sdk\lib\Payments;
use UniversalPay\Payment\Model\Config\Source\DisplayMode;
use Magento\Framework\App\Helper\AbstractHelper;
use UniversalPay\Payment\Model\Config\Source\Environment;
/**
 * @SuppressWarnings(PHPMD.LongVariable)
 */
class Helper extends AbstractHelper
{
    const METHOD_CODE = 'universalpay_payment';
    ...
    {
        $writer = new \Zend\Log\Writer\Stream(BP . '/var/log/universalpay.log');
    ...
    public function getNotificationRoute($orderId)
    {
        $displayMode = $this->getConfigData('display_mode');
        if ($displayMode === DisplayMode::DISPLAY_MODE_REDIRECT) {
            return 'universalpay/hosted/callback/orderid/' . $orderId;
        } else if ($displayMode === DisplayMode::DISPLAY_MODE_HOSTEDPAY) {
            return 'universalpay/hosted/callback/orderid/' . $orderId;
        }else if ($displayMode === DisplayMode::DISPLAY_MODE_EMBEDDED) {
    ...
    public function getLandingPageOnReturnAfterRedirect($orderId)
    {
        $displayMode = $this->getConfigData('display_mode');
        if ($displayMode === DisplayMode::DISPLAY_MODE_REDIRECT) {
            return 'universalpay/hosted/response/orderid/' . $orderId;
        } else if ($displayMode === DisplayMode::DISPLAY_MODE_HOSTEDPAY) {
            return 'universalpay/hosted/response/orderid/' . $orderId;
        }else if ($displayMode === DisplayMode::DISPLAY_MODE_EMBEDDED) {
